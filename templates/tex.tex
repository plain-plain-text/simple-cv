...
%%% This font declaration looks for metadata values set on the fly
%%% in tmp/metadata.yml, regular-found, bold-found, italic-found, and smallcaps-found.
%%% They, in turn, rely on the results of fc-list, which is a kludgy
%%% way of finding which fonts fontspec knows about.
$if(regular-found)$
\setmainfont{$pdf-fonts.regular$}[%
  $if(italic-found)$
    ItalicFont = $pdf-fonts.italic$ ,
  $endif$
  $if(bold-found)$
    BoldFont = $pdf-fonts.bold$ ,
  $endif$
  $if(pdf-bold-as-smallcaps)$
    BoldFeatures = {Letters=SmallCaps} ,
  $endif$
  $if(smallcaps-found)$
    SmallCapsFont = $pdf-fonts.smallcaps$ ,
  $endif$
